...
impl ZaiLanguageModelProvider {
    ...
    fn settings(cx: &App) -> &ZaiSettings {
        &crate::AllLanguageModelSettings::get_global(cx).z_ai
    }
...
impl LanguageModelProvider for ZaiLanguageModelProvider {
    ...
    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        ...
        for model in z_ai::Model::iter() {
            if !matches!(model, z_ai::Model::Custom { .. }) {
    ...
    fn new(state: Entity<State>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        let api_key_editor = cx.new(|cx| {
            SingleLineInput::new(
                window,
                cx,
                "zai-0000000000000000000000000000000000000000000000000",
            )
            .label("API key")
        });
